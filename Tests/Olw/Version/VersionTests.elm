...
initialVersionDoc =
    Build.beginDocument (Version.buildRootVersionNode "0a")
...
largerExampleVersionTree =
    initialVersionDoc
        ...
        -- node 2
        |> Result.andThen (update 2 "2a")
        -- node 3
        |> Result.andThen (update 2 "2b")
...
versionTest =
    describe "Version"
        ...
        , test "lsaPathFromRootTo element returns correct path (0 1)" <|
            \() ->
                largerExampleVersionTree
                    |> Result.map (lsaPathFromRootTo 1)
                    |> Expect.equal (Ok [ 0, 1 ])
        , test "lsaPathFromRootTo element returns correct path (0 2 4)" <|
            \() ->
                largerExampleVersionTree
                    |> Result.map (lsaPathFromRootTo 4)
